...
const MAX_FILE_SIZE = 2 * 1024 * 1024 * 1024; // 2GB (Vercel Blob supports up to 5TB with multipart)

export function VideoUploadForm({ acts, onSuccess, onError }: VideoUploadFormProps) {
  ...
  const validateFile = (file: File): string | null => {
    try {
      ...
      return null; // Let server validate
    }
  };
  ...
  const handleFileSelect = (file: File) => {
    ...
    // Store file info immediately - we'll upload directly from the File object
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      setErrors({ ...errors, file: `File selection error: ${err instanceof Error ? err.message : String(err)}` });
    }
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const sanitizedName = formData.fileName.replace(/[^a-zA-Z0-9.-]/g, '_');
      ...
      const blob = await upload(pathname, formData.file, {
        ...
        multipart: true, // Enables large file support with chunked uploads
      ...
      // Step 2: Create queue entry with the blob URL
      ...
      const queueResponse = await fetch('/api/upload/queue', {
        ...
        body: JSON.stringify({
          ...
          actIds: formData.actIds,
          performerIds: formData.performerIds,
        }),
      });
  ...
  const removeFile = () => {
    setFormData({ ...formData, file: null, fileName: '', fileSize: 0 });
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {isSubmitting && (
              <div className="mt-3">
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-text-secondary">{getProgressText()}</span>
      ...
      <div className="pt-2">
        <Button
          ...
          className="w-full"
        >
          {isSubmitting ? getProgressText() : 'Upload Video'}
        </Button>
